...
#include "ui_notifimpospenalidadeprocedencia.h"

#include "mainwindow.h"
#include "ui_mainwindow.h"

NotifImposPenalidadeProcedencia::NotifImposPenalidadeProcedencia(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::NotifImposPenalidadeProcedencia)
{
    ui->setupUi(this);
...
NotifImposPenalidadeProcedencia::~NotifImposPenalidadeProcedencia()
{
    delete ui;
}

void NotifImposPenalidadeProcedencia::on_checkBoxInterdicaoParcialEstab_stateChanged(int)
{
...
QStringList NotifImposPenalidadeProcedencia::getVariablesFromLayout()
{
    ...
    imposicaoMensagemRetif, suspensaoPropaganda};
    /*
     * 0 - numAuto
     * 1 - numProcesso
     * 2 - nomeServidor
...
QStringList NotifImposPenalidadeProcedencia::converteQDateEmStringList(QDate date)
{
...
void MainWindow::notificacaoImposPenalidadeProcedenciaODT(QString fn)
{
    setupEstruturaODT();

    //QFile file("modelos/Notificação de Imposição de Penalidade - Procedência da Autuação ODT/content.xml");
    ...
    stream.writeCharacters(QString("Pela presente fica notificado(a) %1, CPF/CNPJ %2, situado na %3, CEP %4, no Município de %5, da imposição da(s) penalidade(s) acima assinalada(s), em razão do julgamento de procedência do Auto de Infração Sanitária nº %6, de %7 de %8 de %9, e que, de acordo com o art. 30 da Lei Federal nº 6437/77, terá prazo de 15 (quinze) dias a partir do recebimento desta para, querendo, apresentar recurso perante ")
    .arg(estabRazaoSocial).arg(estabCnpjCpf).arg(estabEndereco).arg(estabCEP).arg(estabCidade).arg(numAuto).arg(diaAuto).arg(mesAuto).arg(anoAuto));
    stream.writeEndElement(); // text:span
    ...
    //QString dirToCompress = "modelos/Notificação de Imposição de Penalidade - Procedência da Autuação ODT";
...
void MainWindow::notificacaoImposPenalidadeProcedenciaDOCX(QString fn)
{
    setupEstruturaDOCX();

    //QFile file("modelos/Notificação de Imposição de Penalidade - Procedência da Autuação DOCX/word/document.xml");
    ...
    stream.writeStartElement("w:t");
    stream.writeAttribute("xml:space", "preserve");
    stream.writeCharacters(QString("Pela presente fica notificado(a) %1, CPF/CNPJ %2, situado na %3, CEP %4, no Município de %5, da imposição da(s) penalidade(s) acima assinalada(s), em razão do julgamento de procedência do Auto de Infração Sanitária nº %6, de %7 de %8 de %9, e que, de acordo com o art. 30 da Lei Federal nº 6437/77, terá prazo de 15 (quinze) dias a partir do recebimento desta para, querendo, apresentar recurso perante %10")
    .arg(estabRazaoSocial).arg(estabCnpjCpf).arg(estabEndereco).arg(estabCEP).arg(estabCidade).arg(numAuto).arg(diaAuto).arg(mesAuto).arg(anoAuto).arg(nomeEquipeFormatado));
    stream.writeEndElement(); // w:t
    stream.writeEndElement(); // w:r
    ...
    //QString dirToCompress = "modelos/Notificação de Imposição de Penalidade - Procedência da Autuação DOCX";
